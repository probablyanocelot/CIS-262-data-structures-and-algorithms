"""
3. When you send a file to be printed on a shared printer, it is put onto a print 
...
the queue implementations. The method should expect an item as an argument. 
It should remove the given item in the queue or raise an exception if the item is 
not found.
"""
...
class QueueSelector:
    ...
    def smallest(self):
        """
        Selects the queue with the smallest number of items in it.
        """
        smallest_queue = None
        smallest_size = float('inf')
        
        for queue in self.queue_list:
            if len(queue) < smallest_size:
                smallest_size = len(queue)
                smallest_queue = queue
        
        return smallest_queue
        
    def add_to_smallest(self, item):
        """
        Adds an item to the queue with the smallest number of items in it.
        """
        smallest_queue = self.smallest()
        smallest_queue.add(item)
...
selector = QueueSelector([q, q2, q3])
print(f"Smallest queue contents: {selector.smallest()}")
print(f"Adding 6 to smallest queue")
selector.add_to_smallest(6)
print(f"Smallest queue contents: {selector.smallest()}")